...
Desk::Desk (const QString &dirPath, const QString &trashPath, int sizeWidth,
   ...
   _dir = dirPath;
   _rootDir = trashPath;
   _rightMargin = sizeWidth - 150;
   qDebug() << "Width: " << sizeWidth;
   if (!_dir.isEmpty ())
   ...
   if (!trashPath.isEmpty ())
      ...
      if (!dir.exists (_trash_dir)){
         ...
         //SetFileAttributes(_trash_dir.toLocal8Bit().constData(),FILE_ATTRIBUTE_HIDDEN);
      }
   ...
   for (int i = 0; i < list.size (); i++)
      ...
      if (fi.isDir ())
         ...
         if (fi.fileName () != "." && fi.fileName () != "..")
          ...
          if(fi.fileName().endsWith(".pdf") || fi.fileName().endsWith(".max")
                  || fi.fileName().endsWith(".jpg") || fi.fileName().endsWith(".jpeg"))
              addFile (fi, dirPath);
        // qDebug() << "Adding: " << fi.fileName();
      }
      }
   updateRowCount ();
   }
...
void Desk::arrangeBy (int type)
   ...
   foreach (f, _files)
      ...
      qDebug ()<< "OldPos: " << f->filename() << " : " << f->pos();
...
void Desk::updatePosResize (int newWidth, QString match)
   {


    int i = _files.size();
   // qDebug () << "Hereeeeee: " << newWidth;
//return;
    if(i == 0)return;
  //  qDebug() << "here1";
   File *f;
   _rightMargin = newWidth - 150;
   resetPos ();
 //  qDebug() << "here2";
...
err_info *Desk::moveFromTrash (QString &trashname, QString &filename,
   ...
   //QString fullPath = _trash_dir + trashname;
   //qDebug() << "Here: " << fullPath;
   return moveFromDir (_trash_dir, trashname , filename, fnew);
   }


err_info *Desk::moveFromDir (QString & d, QString &trashname, QString &filename,
      File *&fnew)
   {
   QString ext;
...
void Desk::removeRows (int row, int count)
   {
   // qDebug () << "passed this " << count ;
   for (int i = 0; i < count; i++)
    //  qDebug() << _files.takeAt(i)->pathname();
      _files.takeAt (row);
   }
