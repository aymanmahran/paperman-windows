...
void Pdfio::setPathname (QString rename){
    _pathname = rename;
    return;
}
...
err_info *Pdfio::open (void)
   ...
   try
      ...
      _doc->Load (_pathname.toLatin1 ().constData());
      }
...
err_info *Pdfio::close (void)
   ...
   try
      {
        //printf(_pathname.toLatin1 ().constData());
        qDebug () << "saving...";
        //delete _doc;
       _doc->Write ((_pathname).toLatin1 ().constData());
       // _doc->~PdfMemDocument();
       // _doc->Write ("F:/Ayman/Desktop/Downloads/testoutputfile.pdf");
      }
   catch (const PdfError &eCode)
      {
      return make_error (eCode);
   ...
   try
      {
        ...
        delete _doc;
        _doc = 0;
       // _doc->WriteUpdate ((_pathname).toLatin1 ().constData());
       // _doc->~PdfMemDocument();
       // _doc->Write ("F:/Ayman/Desktop/Downloads/testoutputfile.pdf");
      }
   catch (const PdfError &eCode)
...
err_info *Pdfio::appendFrom (Pdfio *from)
{
   ...
   mytry
      ...
      // _doc->InsertPages (*from->_doc, 1, from->_doc->GetPageCount());
     //  _doc->Write ((_pathname).toLatin1 ().constData());
...
err_info *Pdfio::insertPages (Pdfio *from, int start, int count)
{
   mytry
      ...
      //Poppler::Page* pdfPage = _pop->page(start);
      }
#ifdef EXCEPTIONS
   catch (const PdfError &eCode)
      {
