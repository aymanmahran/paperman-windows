...
//          bool delay_smoothing = false);
   ...
   void showPages (const QAbstractItemModel *model, const QModelIndex &index,
         int start, int count, int across, bool reset = false, const char* str = __builtin_FUNCTION());
   ...
   //bool des(const int &v1, const int &v2){return v1 > v2;}


signals:

   //Save selected item to be selected after update
   void saveSelected();
...
public slots:
   ...
   void slotSetTarget (const QModelIndex &index, const QModelIndex &list);
...
//    QGraphicsView *_view;
   ...
   QWidget *_ocr_area;
   QWidget *_ocr_toggle_widget;
   QPushButton *_ocr_toggle;
   bool _ocr_status;
   int _ocr_size;
