...
public:
   ...
   virtual void *kill (void) = 0;  // was desk->ensureMax
...
//             QString &newname, QString &pagename, File **dest);
   ...
   void setPos (QPoint pos, const char* str = __builtin_FUNCTION());
...
//    err_info *moveToTrash (QString &trashname);
   ...
   static err_info *not_impl (const char *str = __builtin_FUNCTION());
   ...
   err_info *reorderItem (int num, int list);
   ...
   err_info *unstackTrash (int pagenum, int pagecount, bool remove,
               QString fname, QString &dest, int &itemnum, int seq = -1);
