...
void UCUnstackPage::redo (void)
   ...
   //complain (_model->opReorder (1 , 2, index));
...
UCTrashFirst::UCTrashFirst (Desktopmodel *model, QModelIndex &ind,
      int pagenum, bool remove)
      : Desktopundocmd (model)
   {
   _dir = model->deskToDirname (ind.parent ());
   ...
   _pagenum = pagenum;
   _remove = remove;
   setText (QApplication::translate("UCTrashFirst",
      "Delete first page"));
   }


void UCTrashFirst::redo (void)
   {
   QModelIndex parent = _model->deskFromDirname (_dir);
   QModelIndex index = _model->index (_filename, parent);

   complain (_model->opUnstackTrash (index, _pagenum, _remove, _newname));
   }


void UCTrashFirst::undo (void)
   {

   // qDebug () << "Newname: " << _newname ;
   // return;
   QModelIndex parent = _model->deskFromDirname (_dir);
   QModelIndex index = _model->index (_filename, parent);
   //QModelIndex page = _model->index (_newname, parent);
  ...
  // list << page;
   ...
   // if we can't find something, skip
   QFile f(_newname);
   if (index == QModelIndex () || !f.exists())
      return;

   complain (_model->opStackTrash (index, _newname));

   }

UCReorder::UCReorder (Desktopmodel *model, QModelIndex &ind,
      int num, int list)
      : Desktopundocmd (model)
   {
   _dir = model->deskToDirname (ind.parent ());
   ...
   _num = num;
   _list = list;
   setText (QApplication::translate("UCReorder",
      "Reorder"));
   }


void UCReorder::redo (void)
   {
   QModelIndex parent = _model->deskFromDirname (_dir);
   QModelIndex index = _model->index (_filename, parent);
   qDebug() << _filename;
   complain (_model->opReorder (_num, _list, index));
   }


void UCReorder::undo (void)
   {
    return;
   }


UCSendTo::UCSendTo (Desktopmodel *model, QModelIndex &ind,
      QString dir)
      : Desktopundocmd (model)
   {
   _dir = model->deskToDirname (ind.parent ());
   _filename = model->data (ind, Desktopmodel::Role_filename).toString ();
   //_filedir = model->data(ind, Desktopmodel::Role_pathname).toString();
   _filedir = _dir;
   _newdir = dir;
   setText (QApplication::translate("UCSendTo",
      "Send stack"));
   }


void UCSendTo::redo (void)
   {
   QModelIndex parent = _model->deskFromDirname (_dir);
   QModelIndex index = _model->index (_filename, parent);

   complain (_model->opSendTo(index, parent, _newdir, _newname));
   }


void UCSendTo::undo (void)
   {
...
//      return;
    ...
    complain (_model->opUnsendTo(_newdir, _newname, _filedir));
    _filename = _newname;
   }
...
void UCChangeDir::redo (void)
   {
   _model->opChangeDir (_newpath, _newroot, _sizewidth);
   }


void UCChangeDir::undo (void)
   {
   _model->opChangeDir (_oldpath, _oldroot, _sizewidth);
   }
