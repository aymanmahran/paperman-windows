...
void Desktopmodel::trashFirst (QModelIndex &ind, int pagenum, bool remove)
   {
   QModelIndexList list;
   ...
   list << ind;
   if (checkScanStack (list, ind.parent ()))
      _undo->push (new UCTrashFirst (this, ind, pagenum, remove));
   }

void Desktopmodel::sendTo (QModelIndex &ind, QString dir)
   {
   QModelIndexList list;
   ...
   list << ind;
   if (checkScanStack (list, ind.parent ()))
      _undo->push (new UCSendTo (this, ind, dir));
   }

void Desktopmodel::reorder (int num, int list, QModelIndex &ind)
   {
   QModelIndexList lit;
   ...
   if (checkScanStack (lit, ind.parent ()))
      _undo->push (new UCReorder (this, ind, num, list));
...
void Desktopmodel::changeDir (QString dirPath, QString rootPath,
         bool allow_undo, int sizeWidth)
   {
   if (allow_undo)
      _undo->push (new UCChangeDir (this, dirPath, rootPath, _dirPath, _rootPath, sizeWidth));
   else
      opChangeDir (dirPath, rootPath, sizeWidth);
   }
