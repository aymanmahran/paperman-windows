...
void File::setPos (QPoint pos, const char* str)
   {
   /* QT crashes when any position is -ve because its
    ...
    /*qDebug () << " Called from: " << str;

    if( QString::compare(str, "updatePosResize", Qt::CaseInsensitive) == 0){
        qDebug () << "Old: " << pos;
    }*/
   if (pos.x () < 0)
      pos.setX (0);
...
err_info *File::unstackItems (int pagenum, int pagecount, bool remove,
   ...
   kill();
   HummusPDFCore p;
   //QString baseFileName = removeExtension (filename(), ext);
   int status=p.split(pathname().toStdString(),_dir.toStdString(),pagename.toStdString(),false);
   if (status)
      qDebug() << "Error here: " << (tr(p.getErrorMsg().c_str()));
...
err_info *File::reorderItem(int num, int list){
    ...
    kill();
   // qDebug () << "DOing";
    int status=p.reorder(num, list, pathname().toStdString());
    //qDebug () << "DOing2";
...
err_info *File::unstackTrash (int pagenum, int pagecount, bool remove,
               QString fname, QString &dest, int &itemnum, int seq)
   {
   QString uniq, str;
  ...
  // CALL (dest->create ());
   ...
   QString old_fname = pagename;
 //  QFile file (_pathname);

   QString newDir = _desk->trashdir ();

   // get the full paths for the current file, and the new file it will become
   QString oldPath = _dir + "/" + pagename + ".pdf";
   QString newPath = newDir + "/" + pagename + ".pdf";

   // the new name will hopefully be the same as the old
   QString newName = pagename;
...
err_info *File::not_impl (const char *str)
   {
    qDebug() << "Called from: " << str;
   return err_make (ERRFN, ERR_no_available_for_this_file_type);
   // return err_make (ERRFN, ERR_ok);
