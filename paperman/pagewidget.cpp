...
//    checkSubsystem (xmlConfig->intValue("DISPLAY_SUBSYSTEM"));
   ...
   connect (_tools->sendNow, SIGNAL (clicked ()), this, SLOT (slotSendNow ()));
...
//    layout->addWidget (_stack);
   ...
   _ocr_toggle = new QPushButton();
   //button->setText("Test");

   QPixmap col(":/images/images/collapse.xpm");
   //QPixmap col("C:/Users/Ayman/Desktop/collapse.xpm");
   QIcon coli(col);
   /*button->setIcon(coli);
   button->setIconSize(QSize(5, 5));
   button->setMinimumSize(0, 0);
   button->setSizePolicy(QSizePolicy::Fixed);*/
   _ocr_toggle->setStyleSheet("border-image:url(:/images/images/collapse.xpm);");
   _ocr_toggle->setFixedSize(10, 10);

   _ocr_toggle_widget = new QWidget ();

   QHBoxLayout *t = new QHBoxLayout();

   t->addWidget(new QLabel("OCR"));
  // t->setAlignment(button, Qt::AlignRight);
   t->addWidget(_ocr_toggle, 0, Qt::AlignRight);
   t->setContentsMargins(0, 0, 0, 0);
   t->setMargin(0);
  // t->setSizeConstraint(QLayout::SetFixedSize);


   _ocr_toggle_widget->setLayout(t);
   ...
   lay3->addWidget (_ocr_toggle_widget);
   lay3->addWidget (_ocr_area);
   QWidget *fin = new QWidget ();
   fin->setLayout (lay3);


   connect(_ocr_toggle, SIGNAL(clicked(bool)), this, SLOT(onECClicked(bool)));
   //ocrToggle->show();
...
//       this, SLOT (slotPreviewPage (const QModelIndex &)));
   ...
   connect (_pageview, SIGNAL (setTargetNum (const QModelIndex &, const QModelIndex &)),
      this, SLOT (slotSetTarget (const QModelIndex &, const QModelIndex &)));
   ...
   connect (_pageview, SIGNAL (setNum(int)),
      _pagemodel, SLOT (slotSetNum(int)));
...
void Pagewidget::onECClicked(bool ok){
    //qDebug() << "testsssss";

    if(!_ocr_status){
        _ocr_area->setMinimumSize(0, 0);
        _ocr_area->setMaximumSize(0, 0);
        _ocr_toggle->setStyleSheet("border-image:url(:/images/images/expand.xpm);");
        QList<int> sizes = _ocr_split->sizes ();
        QList<int> colsizes;
        _ocr_size = sizes[1];
        colsizes.append(sizes[0]+sizes[1]-_ocr_toggle_widget->size().height());
        colsizes.append(_ocr_toggle_widget->size().height());
        _ocr_split->setSizes(colsizes);
        _ocr_status = true;
    }
        ...
        QList<int> sizes = _ocr_split->sizes ();
        QList<int> colsizes;
        colsizes.append(sizes[0]+sizes[1]-_ocr_size);
        colsizes.append(_ocr_size);
        _ocr_area->setMinimumSize(0,0);
        _ocr_area->setMaximumSize(16777215, 16777215);
        _ocr_split->setSizes(colsizes);
        _ocr_status = false;
    }
...
void Pagewidget::updatePagetools (void)
   ...
   _tools->sendNow->setEnabled (have_page);
...
//    closing ();
   ...
   emit modeChanging (mode, Mode_select);
   ...
   if (_mode != Mode_none)
      ...
      QString str = QString ("%1mode%2/").arg (_settings_base).arg (Mode_select);
      QList<int> size;
...
//         _area->viewport ()->resize (widgetSize);
      ...
      //   qDebug() << "Sizes: " << widgetSize << " : " << areaSize;

         if(_rotate % 180 == 90){
          //   int n = 100;
             _area->verticalScrollBar()->setPageStep(areaSize.width());
             _area->horizontalScrollBar()->setPageStep(areaSize.height());
             _area->horizontalScrollBar()->setRange(0, widgetSize.height() - areaSize.width());
             _area->verticalScrollBar()->setRange(0, widgetSize.width() - areaSize.height());

//             QSize sz(areaSize.width() + n, areaSize.height() - n);
//             _area->viewport()->resize(sz);
...
//         printf ("updateScale = %d\n", updateScale);
         if (updateScale)
            ...
            if (!_too_big && _pixmap.size () != finalSize)
               {
                qDebug() << "Here 1 " << finalSize << " " << _pixmap.size();
...
void Pagewidget::showPages (const QAbstractItemModel *model, const QModelIndex &index,
         int start, int count, int curpage, bool reset, const char* str)
   {

    slotReset();
    qDebug ()<< "showPages: " << str;
...
void Pagewidget::slotSetTarget (const QModelIndex &index, const QModelIndex &list)
   {
    qDebug() << "Here: ";
    int insertAt, newPage;
    if(index != QModelIndex()){
        QVariant value = index.model ()->data (index, Pagemodel::Role_pageinfo);
        Pageinfo *pi = value.value<Pageinfo *>();
        insertAt = pi->pagenum();

        value = list.model ()->data (list, Pagemodel::Role_pageinfo);
        pi = value.value<Pageinfo *>();
        newPage = pi->pagenum();

        emit (reorder(insertAt, newPage));

        //qSort(newPages.begin(), newPages.end(), des);
       // std::sort(newPages.begin(), newPages.end());
       // std::reverse(newPages.begin(), newPages.end());



        //qDebug() << "Num: " << index.model ()->data (index, Pagemodel::Role_target).toInt();
...
void Pagewidget::slotSendNow (void){
    emit sendSelected();
}
...
{
   // QSize sz(_area->viewport()->size().width() + 20, _area->viewport()->size().height() - 20);
  //  _area->viewport()->resize(sz);
  //  _area->setSize(sz);
   addRotate (90);
}
...
void MyScrollArea::paintEvent (QPaintEvent *event)
   ...
   if(_rotate % 180 == 60){
       exposedRect = m
                    .translate (-verticalScrollBar()->value (), -horizontalScrollBar()->value ())
                    .inverted ()
                    .mapRect (event->rect ())
                    .adjusted (-1, -1, 1, 1);
   }
  ...
  // qDebug () << "exposed" << exposedRect << "image" << _pixmap.size ();

  QPointF target = QPointF (0, 0);
   if(_rotate == 90 )
       target = QPointF (0, -exposedRect.width());
...
//   QPointF center(exposedRect.width() / 2., exposedRect.height() / 2.); // widget center
...
//       brect.setRight (_image->width ());
      ...
      painter.drawPixmap(tp, _pixmap, exposedRect);

     /* if(_rotate % 180 == 90){
          rrect.setLeft (_pixmap.width () - verticalScrollBar()->value ());
          rrect.setTop (-horizontalScrollBar()->value ());
