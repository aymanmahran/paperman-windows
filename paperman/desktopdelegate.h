...
public:
   ...
   void setCurPos(QPoint pos){_curpos = pos;}
...
signals:
   ...
   void itemSendTo(const QModelIndex & ind);

   void itemTrashFirst(const QModelIndex & ind);

   void updateCurPos ()const;
...
private:
   ...
   void measureItem (const QStyleOptionViewItem &option, const QModelIndex &index,
      measure_info &measure, QPoint *offset, bool size_only = false, const char* str = __builtin_FUNCTION()) const;
