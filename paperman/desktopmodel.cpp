...
void Desktopmodel::bld(QModelIndex &ind){
    buildItem(ind);
}

void Desktopmodel::buildItem (QModelIndex index, const char* str)
   {
    qDebug () << "buildItem" << getFile(index)->pathname() << " : " << str;
...
err_info *Desktopmodel::scanCommitPages (QModelIndex &ind, int del_count,
      QBitArray &pages, bool allow_undo)
   {
    ...
    qDebug()<< "dsdffffffffffffff" << _scan_err << " " << del_count;
...
QModelIndex Desktopmodel::refresh (QString dirPath, QString rootPath, int sizeWidth, bool do_readDesk,
   ...
   if (ind.isValid ())
      ...
      //qDebug () << "Creating a new desk with width: " << sizeWidth;
      desk = new Desk (subdirs ? "" : dirPath, rootPath, sizeWidth, do_readDesk);
      _desks << desk;
      endInsertRows ();
...
void Desktopmodel::addFilesToDesk (QString dir, QStringList &fnamelist)
 ...
 //  qDebug () << "addFilesToDesk" << dir << parent;
   // QModelIndex parent = deskFromDirname(dir);

   // if we haven't got a desk for this, don't worry

  //  if(parent == QModelIndex())qDebug() << "Not working";
   if (!parent.isValid ())
      return;
   ...
   qDebug () << "Addding to this desllll" << desk->dir ();
   desk->advance ();
...
bool Desktopproxy::filterAcceptsRow (int source_row, const QModelIndex& source_parent) const
   ...
   //QString nm = fileModel->fileName(index0);
   //QString nm = index0.data(QFileSystemModel.FileNameRole);
    File *f = (File *)index0.internalPointer ();
    QString nm = f->filename();
   //qDebug () << "Filename: " << nm;
   if (!nm.endsWith(".pdf") && !nm.endsWith(".jpg") && !nm.endsWith(".jpeg") && !nm.endsWith(".max")){
       return false;
   }
