...
//	   printf ("test error %s\n", e->errstr);
     ...
     //QToolBar *tbar = me->toolBar;

     //MyEditableLabel el("Test");
     //el.setText("Test");
...
//p          me->_main->getViewer ()->setDebugLevel (debug);
	 ...
	 //	 while (optind < argc)
         ...
         me->setArrangeBy(qs.value("arrangeby").toInt());


        // qDebug() << "Here: " << qs.value("arrangeby").toInt();
     ...
     desktop->setSend(qs.value("sendto").toString());
     desktop->setEmail(qs.value("sendemail").toString());

	 me->show ();

         QModelIndex ind = QModelIndex ();
         desktop->selectDir (ind);
     //    me->_main->arrangeBy(Mainwidget::byName);
         if (err_list.size ())
            {
            QString msg;
