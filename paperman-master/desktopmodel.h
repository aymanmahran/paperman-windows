...
class Desktopmodel : public QAbstractItemModel
   ...
   friend class UCTrashFirst;
   friend class UCReorder;
   friend class UCSendTo;
...
*/
   ...
   void trashFirst (QModelIndex &ind, int pagenum, bool remove);

   void sendTo (QModelIndex &ind, QString dir);

   void reorder (int num, int list, QModelIndex &ind);
...
// maybe implement this later
   ...
   void changeDir (QString dirPath, QString rootPath, bool allow_undo, int sizeWidth);
...
protected:
   ...
   err_info *opUnstackTrash (QModelIndex &index, int &pagenum,
      bool remove, QString &newname);

   err_info *opStackTrash (QModelIndex &index, QString &newname);

   err_info *opSendTo (QModelIndex &index, QModelIndex &parent , QString newdir, QString &newname);

   err_info *opUnsendTo (QString newdir, QString &newname, QString olddir);
   ...
   /** Change directory operation
      ...
      \param dirpath    directory to search
      \param rootpath   root directory for this branch of the model */
   void opChangeDir (QString &dirPath, QString &rootPath, int sizeWidth);
...
public:
   ...
   err_info *opReorder (int num, int list, QModelIndex &ind);

   void bld(QModelIndex &ind);
...
signals:
   ...
   void selectIt (QModelIndex &deskInd);
...
public slots:
   ...
   QModelIndex refresh (QString dirpath, QString rootpath, int sizeWidth, bool do_readDesk = true,
...
private:
   ...
   /** open the file at the given row in the list, get its pixmap and update
       the view */
   void buildItem (QModelIndex index, const char* str = __builtin_FUNCTION());
...
protected:
   ...
   /** email a set of files
      ...
      \returns error or NULL if ok */
   err_info *emailFiles (QString &fname, QStringList &fnamelist, bool &can_delete, QString receiver);
