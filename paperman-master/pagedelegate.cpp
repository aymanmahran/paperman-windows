...
void Pagedelegate::measureItem (const QStyleOptionViewItem &option, const QModelIndex &index,
      pmeasure_info &measure, QPoint *offset, const char* str) const
   {
   ...
   measure.target = index.model ()->data (index, Pagemodel::Role_target).toInt();

   //qDebug()<< "Target: " << measure.target;
   ...
   /*if(measure.target != -1) qDebug() << "Comp: " << measure.target << " : " << measure.pagenum;
   if((measure.pagenum >= measure.target) && measure.target != -1 && (((measure.pagenum -1) != measure.target))){
       qDebug() << "Yes";
       offset->setY(offset->y() + measure.rect.height());
   }*/
   ...
   //measure.position = QPoint(measure.rect.x(), measure.rect.y());
   //model->setData (index, measure.position, Pagemodel::Role_position);
   }
