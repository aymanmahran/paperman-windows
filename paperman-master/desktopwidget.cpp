...
//    _model->setLazyChildCount (true);
   ...
   //QToolBar *tb = new QToolBar(group);
   //MyEditableLabel el("Test", tb);
   //tb->addWidget(new QLabel("Tt"));
   //tb->addWidget(&el);
   ...
   //QWidget *gp = new QWidget (this);
   createSetbar(group);

   connect(_dir_list,SIGNAL(activated(int)),this,SLOT(slotSelectionChanged(int)));

   lay->addWidget(_setbar);
   ...
   connect (_view, SIGNAL (updateResize (int)),
         this, SLOT (slotUpdatePosResize(int)));
...
//   connect (_view, SIGNAL (leaveEvent()), _page, SLOT (slotReset ()));
   ...
   connect (_delegate, SIGNAL (updateCurPos()),
      this, SLOT (slotUpdateCurPos()));
   connect (_delegate, SIGNAL (itemSendTo(const QModelIndex &)),
      this, SLOT (slotItemSendTo(const QModelIndex &)));
   connect (_delegate, SIGNAL (itemTrashFirst(const QModelIndex &)),
      this, SLOT (slotItemTrashFirst(const QModelIndex &)));
  ...
  // connect (_contents, SIGNAL (selectIt (QModelIndex &)),
   //   this, SLOT (slotSelectIt (QModelIndex &)));
   connect (_contents, SIGNAL (selectIt (QModelIndex &)), _view, SLOT (setPositions ()));
   ...
   connect (_dir->_set_send_to, SIGNAL (triggered ()), this,
            SLOT (slotSetSend ()));
...
void Desktopwidget::createPage(void)
   ...
   connect (_page, SIGNAL (sendSelected()),
      this, SLOT (email ()));

   connect (_page, SIGNAL (saveSelected()),
      this, SLOT (slotSaveSelected ()));

   connect (_page, SIGNAL (selectSaved()),
      this, SLOT (slotSelectSaved ()));

   connect (_page, SIGNAL (reorder(int, int)),
     this, SLOT (slotReorder (int, int)));
   //  this, SLOT(unstackPage()));
...
//   _toolbar = group;
   ...
   _match->setMinimumSize(QSize(0, 0));
...
void Desktopwidget::createSetbar(QWidget *gb){

    _label_path = new QLabel(_send_path);
    _label_email = new QLabel(_send_email);

    _setbar = new QToolBar(gb);

    QWidget *all = new QWidget (_setbar);

    QHBoxLayout *hboxLayout = new QHBoxLayout();
    hboxLayout->setSpacing(0);
    hboxLayout->setContentsMargins (0, 0, 0, 0);
    all->setLayout (hboxLayout);

    QList<QString> alldirs = _model->getDirs();
    _dir_list = new QComboBox();
    for(int i=0; i<alldirs.size(); i++){
        _dir_list->addItem(alldirs[i]);
    }

    QSpacerItem *spacerItm = new QSpacerItem(10, 20, QSizePolicy::Expanding, QSizePolicy::Minimum);

    hboxLayout->addItem (spacerItm);

    _dir_list->setMinimumSize(50, 0);
    hboxLayout->addWidget(new QLabel("Send directory: "));
    hboxLayout->addWidget(_dir_list);

    QLabel *sp = new QLabel(" ");
    QSizePolicy sizePolicy2(QSizePolicy::Expanding, QSizePolicy::Fixed);
    sizePolicy2.setHorizontalStretch(1);
    sizePolicy2.setVerticalStretch(0);
    sizePolicy2.setHeightForWidth(sp->sizePolicy().hasHeightForWidth());
    sp->setSizePolicy(sizePolicy2);
    sp->setMinimumSize(QSize(50, 0));

    hboxLayout->addWidget (sp);

    _email_field = new QLineEdit;
    //_match->setObjectName ("match");
    _email_field->setSizePolicy(sizePolicy2);
    _email_field->setMinimumSize(50, 0);
    _email_field->setMaximumWidth(160);

    //_match->setDragEnabled(true);

    connect (_email_field, SIGNAL (returnPressed()),
         this, SLOT (slotUpdateEmail ()));

    hboxLayout->addWidget(new QLabel("Default email: "));
    hboxLayout->addWidget(_email_field);
...
void Desktopwidget::closing (void)
   ...
   qDebug() << "This is the view size: " << sizes();
...
void Desktopwidget::slotUpdateCurPos(){
   // qDebug() << _view->mapFromGlobal(QCursor::pos()) << " : " << QCursor::pos();
   // _delegate->setCurPos(mapToGlobal(_view->geometry().topLeft()));
    _delegate->setCurPos(_view->mapFromGlobal(QCursor::pos()));
}

void Desktopwidget::slotSelectionChanged(int i){
    qDebug() << _dir_list->itemText(i);
    _send_path =  _dir_list->itemText(i);
    _label_path->setText("Send directory: " + _dir_list->itemText(i));
}

void Desktopwidget::slotUpdateEmail(){
    //qDebug() << _email_field->text();
    _email_field->setCursorPosition(0);
    _send_email = _email_field->text();
}
...
//    dirname = dir.absPath () + "/";
   ...
   if (index != QModelIndex ())
   ...
   else if (_model->addDir (dirname, ignore_error))
      {
       ...
       _dir_list->setCurrentIndex(_dir_list->findText(_send_path));
...
void Desktopwidget::selectDir (QModelIndex &index, bool order)
   {
...
void Desktopwidget::setArrangeBy(int mode){
    _arrange_by = mode;
}
...
void Desktopwidget::slotUpdateRepositoryList (QString &dirname, bool add_not_delete)
   ...
   if (add_not_delete)
      ...
      if (index != QModelIndex ())
         ...
         for(int i=0; i<alldirs.size(); i++){
             _dir_list->addItem(alldirs[i]);
             if(QString::compare(alldirs[i], _send_path) == 0)flag = true;
         }
         _dir_list->setCurrentIndex(_dir_list->findText(_send_path));
         if(!flag)_send_path = "";
...
void Desktopwidget::slotSetSend() {

    QString dir = _dir->menuGetPath ();
    setSend(dir);
}

void Desktopwidget::slotReorder (int num, int list){
    //qDebug() << "At this";
    //QModelIndex ind = _view->getSelectedItem();

   // File *f = _contents->getFile (ind);
   // QString s = f->filename();
    //f->reorderItem(num, list, ind);
   // qDebug()<< "te " << ind.model ()->data (ind, Desktopmodel::Role_filename).toString ();
    //f->kill();
    ...
    _contents->opReorder(num, list, index);

   // f->setValid(false);
   // f->load();
    //_contents->bld (ind);
    //_contents->reorder(num, list, ind);
    _page->showPages (index.model (), index, 0, -1, -1);

}
void Desktopwidget::slotSaveSelected(){
    _selected_item = _view->getSelectedItem();
}

void Desktopwidget::slotSelectSaved(){
    slotItemPreview(_selected_item, 0, true);
}

void Desktopwidget::setSend(QString dir){
    _send_path = dir;
    _label_path->setText("Send directory: " + dir);
    _dir_list->setCurrentIndex(_dir_list->findText(dir));
}

void Desktopwidget::setEmail(QString email){
    _send_email = email;
    _email_field->setText(email);
    _label_email->setText("Default email: " + email);
}
...
void Desktopwidget::dirSelected (const QModelIndex &index, bool allow_undo, bool order)
   {
   ...
   //qDebug() << "here" << path << Qt::endl << root_path;
//    printf ("dirSelected %s, %s\n", path.latin1 (), _contents->getDirPath ().latin1 ());
   ...
   if (path != _contents->getDirPath () || order)
      {
      _path = path;
      //qDebug () << "Width of view: " << _initwidth;
      _contents->changeDir (path, root_path, allow_undo, _initwidth);
      qDebug () << "Here: " << path;
      _view->arrangeBy(_arrange_by);
      //_view->setPositions();
      }
...
//    qDebug () << "Desktopwidget::slotDirChanged" << _dir->currentIndex () << index;
  ...
  /* Filemodel = new QFileSystemModel(this);
   ...
   //_view->setModel (Filemodel);

   QString strPattern = ".pdf" ;
   QRegExp regExp(strPattern);

   //   _proxy->setFilterRegExp(regExp);
  // _proxy->setFilterFixedString (".pdf");

   //_view->setModel(_proxy);*/


   //QFileSystemModel model;
   //model.setRootPath("C:\\Users\\Ayman\\Downloads\\test");
  // _view->setModel(&model);
  // _view->setRootIndex(model.index("C:\\Users\\Ayman\\Downloads\\test"));
...
//       _updating = true;
      ...
      QModelIndex root = _model->findRoot (index);
      QString root_path = _model->data (root, QDirModel::FilePathRole).toString ();
      QModelIndex sind = _contents->refresh (_path, root_path, _initwidth,
   ...
   /* but otherwise we can just use the proxy model and tell it to change
      ...
      if (ind != QModelIndex ())
          //compressItems();
          slotUpdatePosResize(_view->size().width());

          //_view->viewport()->resize(_view->viewport()->size());
         //_view->repaint();
         _view->scrollTo (ind);
...
void Desktopwidget::slotSelectIt(QModelIndex &deskInd){
    selectDir (deskInd, true);
}

void Desktopwidget::slotUpdatePosResize(int newWidth){

    QModelIndex in = _view->getSelectedItem();

     QString curPath = _contents->getDirPath() + "/";
     QModelIndex deskInd = _contents->deskFromDirname(curPath);
     Desk *curDesk = _contents->getDesk(deskInd);
     //qDebug () << "New size: " << newWidth << " : " << _contents->getDirPath ();
     if(deskInd != QModelIndex ()){
        // qDebug () << "entered";
        curDesk->updatePosResize(newWidth, _match->text());
        _view->arrangeBy(_arrange_by);
       // _view->setPositions();
     }

     slotItemPreview(in, 0, true);
     //setSelectionRange (in.row(), 1);
     return;
}
...
void Desktopwidget::email (void)
{
   ...
   err_complain (_contents->opEmailFiles (parent, slist, File::Type_other, _send_email));
}
...
void Desktopwidget::emailMax (void)
{
   ...
   err_complain (_contents->opEmailFiles (parent, slist, File::Type_max, _send_email));
}
...
void Desktopwidget::emailPdf (void)
{
   ...
   err_complain (_contents->opEmailFiles (parent, slist, File::Type_pdf, _send_email));
}
...
void Desktopwidget::deleteStacks (void)
   ...
   if ( ok == QMessageBox::Ok)
      ...
      // File *f = _contents->getFile(list[0]);
       //QString t = "";
       //QString v = "";
       //err_info *e = _contents->getFile(list[0])->move (v, t, false);
   }
...
void Desktopwidget::slotItemTrashFirst(const QModelIndex &inde){
    qDebug() << "Trash";
    QModelIndex parent = _view->rootIndexSource ();

    //QModelIndex parent = ind.parent ();
   // int count = inde.model ()->rowCount (inde.parent());
    int row = inde.row ();
    ...
    Q_ASSERT (parent == index.parent ());
    _contents->trashFirst (index, -1, true);
    //if (_contents->itemsAdded (parent, start, count))
    //   _view->setSelectionRange (start, count);

}

void Desktopwidget::slotItemSendTo(const QModelIndex &inde){
    qDebug() << "Senddddd";
   ...
   // int count = inde.model ()->rowCount (inde.parent());
    ...
    Q_ASSERT (parent == index.parent ());
    _contents->sendTo (index, _send_path);
    //if (_contents->itemsAdded (parent, start, count))
    //   _view->setSelectionRange (start, count);
