...
static QPixmap *trashfirst = 0;
static QPixmap *sendto = 0;
...
Desktopdelegate::Desktopdelegate (Desktopmodelconv *modelconv, QObject * parent)
   ...
   if (!pleft)
      ...
      trashfirst = new QPixmap (":/images/images/trash_first.xpm");
      *trashfirst = trashfirst->scaled(24, 24, Qt::KeepAspectRatio, Qt::FastTransformation);
      sendto = new QPixmap (":/images/images/send_to.xpm");
      *sendto = sendto->scaled(24, 24, Qt::KeepAspectRatio, Qt::FastTransformation);
...
void Desktopdelegate::measureItem (const QStyleOptionViewItem &option, const QModelIndex &index,
      measure_info &measure, QPoint *offset, bool size_only, const char* str) const
   {
//   Desktopmodel *model = (Desktopmodel *)index.model ();
   ...
   measure.title_maxsize = QSize(120, measure.title_maxsize.height());
   //measure.title_maxsize = QSize(150, measure.title_maxsize.height());
   ...
   //model->setData (index, QSize(100, 142), Desktopmodel::Role_preview_maxsize);

   QSize oldsize = model->data (index, Desktopmodel::Role_preview_maxsize).toSize ();

 //  qDebug () << oldsize.width() << " : " << str << " : " <<measure.valid;
   ...
   /* unfortunately we don't know if an item has multiple pages until we
      ...
      if (size_only)
         ...
         measure.size = QSize (width + 0, 0 + oldsize.height () + measure.title_maxsize.height ()
               + measure.pagename_maxsize.height () + 20 +
...
void sizeHintChanged ( const QModelIndex & index ) */
  ...
  // qDebug () << m;
   ...
   QString elidedTitle = fm.elidedText(measure.title, Qt::ElideMiddle, 120, 0);
   measure.titleRect = style->itemTextRect (fmb, rect, Qt::AlignLeft, false, elidedTitle);
   if (measure.multiple)
      {
  ...
  // measure.size = QSize (width + 0, 0 + size.height () + measure.titleRect.height ()
  //       + measure.pagenameRect.height () + measure.pagenumRect.height () + 20);
...
//          measure.size.width (), measure.size.height ());
   ...
   if (!size_only)
     ...
     // measure.rect = QRect (QPoint (0, 0), QSize(measure.size.width() + 30, measure.size.height()));
      measure.rect = QRect (QPoint (0, 0), measure.size);


      /*
      measure.pixmapRect.setY(measure.pixmapRect.y() + 10);
      measure.pixmapRectHint.setY(measure.pixmapRectHint.y() + 10);

   //   measure.pixmapRect.setHeight(measure.pixmapRect.height() - 10);
      measure.titleRect.setY(measure.titleRect.y() + 10);
   //   measure.titleRect.setHeight(measure.titleRect.height() - 10);

      if(measure.multiple){
          measure.pagenumRect.setY(measure.pagenumRect.y() - 10);
          measure.pagenumRect.setHeight(measure.pagenumRect.height() - 10);
          measure.pagenameRect.setY(measure.pagenameRect.y() + 10);
        //  measure.pagenameRect.setHeight(measure.pagenameRect.height() - 10);
      }
*/
     ...
     // measure.rect.setWidth(measure.rect.width() + 30);

      measure.pixmapRect.translate (off);
      measure.pixmapRectHint.translate (off);
      measure.titleRect.translate (off);
      measure.pagenameRect.translate (off);
      measure.pagenumRect.translate (off2);

      measure.sendto = QRect(measure.pixmapRect.x() + measure.pixmapRect.width() + 6,
                                 measure.pixmapRect.y(),
                                 24, 24);

      if(measure.multiple){

          measure.trashfirst = QRect(measure.pixmapRect.x() + measure.pixmapRect.width() + 6,
                                     measure.pixmapRect.y() + 30,
                                     24, 24);
      }
*/

      measure.sendto = QRect((measure.rect.x() + measure.rect.width())/2 - 12,
                                 measure.pixmapRect.y() + measure.pixmapRect.height() - 34,
                                 24, 24);

      if(measure.multiple){

          measure.sendto = QRect((measure.rect.x() + measure.rect.width())/4 - 12,
                                     measure.pixmapRect.y() + measure.pixmapRect.height() - 34,
                                     24, 24);

          measure.trashfirst = QRect((measure.rect.x() + measure.rect.width())/4 - 12
                                     + (measure.rect.x() + measure.rect.width())/2 ,
                                    measure.pixmapRect.y() + measure.pixmapRect.height() - 34,
                                     24, 24);
      }
      ...
      if (offset)
         ...
         measure.trashfirst.translate(*offset);
         measure.sendto.translate(*offset);
         }
      }
   }
...
void Desktopdelegate::paint (QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
  ...
  //  QPoint curpos = QCursor::pos();
   ...
   //if (!measure.title.endsWith(".pdf"))return;
   painter->save ();
...
// #define OUTLINE
  ...
  // QPoint curNow = QCursor::pos();
  // QPoint mappedPos (_curpos);

 // qDebug () << mappedPos << " : " << measure.rect;

  /* if((option.state & QStyle::State_MouseOver) || (
           //((measure.rect.x() <= mappedPos.x()) && ((measure.rect.x() + measure.rect.width()) >= mappedPos.x())) &&
           //((measure.rect.y() <= mappedPos.y()) && ((measure.rect.y() + measure.rect.height()) >= mappedPos.y()))
             false)){

    //   painter->fillRect (measure.rect, style->standardPalette().color(QPalette::Light));
   //    if(measure.multiple) painter->drawPixmap (measure.trashfirst.x(), measure.trashfirst.y(), *trashfirst);
   //    painter->drawPixmap (measure.sendto.x(), measure.sendto.y(), *sendto);

     }*/

   if (measure.valid)
      style->drawItemPixmap (painter, measure.pixmapRect, Qt::AlignHCenter, measure.pm);
       //style->drawItemPixmap (painter, QRect(measure.pixmapRect.x(), measure.pixmapRect.y(), 100, 142), Qt::AlignHCenter, measure.pm);
   ...
   QFontMetrics fm (option.font);
   QString elidedTitle = fm.elidedText(measure.title, Qt::ElideRight, 110, 0);
   style->drawItemText (painter, measure.titleRect, Qt::AlignHCenter,
         style->standardPalette (), false, elidedTitle,
...
//       printf ("hit = %d\n", _hitwhich);
      ...
      else if (_hitwhich == Point_send){
          //emit itemPreview (_hit, Point_send, true);
          emit itemSendTo(_hit);
      }
      else if (_hitwhich == Point_trash)
      {
          //emit itemPreview (_hit, Point_trash, true);
          emit itemTrashFirst(_hit);
      }
...
//   printf ("%d,%d - %d,%d\n", rect.x (), rect.y (), rect.width (), rect.height ());
   ...
   if (measure.rect.contains (pos))
      ...
      if (ctrl.contains (pos)){
          if (pos.x () < ctrl.left () + pleft->width ())
             which = Point_left;
          ...
          if(measure.sendto.contains(pos))
              which = Point_send;
          else if (measure.trashfirst.contains(pos))
              which = Point_trash;
        }
      }
   return which;
