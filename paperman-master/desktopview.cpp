...
//    printf ("Desktopview = %p\n", this);
 ...
 //   setWrapping(true);
   ...
   SLOT(slotIndexesMoved (const QModelIndexList &));
...
//       printf ("   - item %d\n", i);
     ...
     // model->setData (index, 2, Desktopmodel::Role_pagenum);
...
QSize Desktopview::setPositions (const char* str)
   {
...
//          dest = QModelIndex ();
   ...
   qDebug() << "drop event, dest = " <<  dest.row () << ", " << dropIndicatorPosition ();
...
//    printf ("Desktopview::resizeEvent\n");
  ...
  //  qDebug() << this->pos();
    ...
    if(!_size_flag){
        ...
        emit updateResize(size().width());
    }

    //QModelIndex in = getSelectedItem();

    //qDebug() << "Resize: " << dmodel->getDirPath();
...
void Desktopview::leaveEvent (QEvent *event){
    //qDebug () << "Leaving: ";
    //mapFromGlobal(QCursor::pos());
    QListView::leaveEvent(event);
}

void Desktopview::enterEvent (QEvent *event){
    //qDebug () << "Entering: ";
...
int Desktopview::getSelectionSummary (void)
   ...
   if (list.size () > 1){
       ...
       foreach (ind, list)
          {
           if(!ind.model ()->data (ind, Desktopmodel::Role_type).toBool()) f = 1;
          }
       if (f == 0) _sel_summary |= SEL_more_than_one;
   }
