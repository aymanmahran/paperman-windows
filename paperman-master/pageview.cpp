...
void Pageview::dropEvent (QDropEvent* event)
   ...
   //needs reimplement
   QModelIndex dest = indexAt (event->pos ());
   int n = dest.model()->data(dest, Qt::DisplayRole).toInt();
   qDebug() << "Off: " << n;
   emit(setNum(n));

//   QAbstractItemModel *model1 = (QAbstractItemModel *)dest.model ();
   QModelIndexList list = selectedIndexes ();
//   QAbstractItemModel *model2 = (QAbstractItemModel *)list[0].model ();

//   int n = dest.model()->data(dest, Qt::DisplayRole).toInt();
//   int m = list[0].model()->data(list[0], Qt::DisplayRole).toInt();

//   model1->removeRows(n, 1, QModelIndex());
//   model1->insertRows(n, 1, QModelIndex());

//   model1->removeRows(m, 1, QModelIndex());
//   model1->insertRows(m, 1, QModelIndex());

//   model1->setData(dest, m, Qt::DisplayRole);
//   model2->setData(list[0], n, Qt::DisplayRole);
 ...
 //  foreach (ind, list)
  ...
  // QListView::dropEvent (event);
   ...
   if (dest == QModelIndex ())
       ...
       emit(setTargetNum(dest, list[0]));
       QListView::dropEvent (event);
       //emit(setTargetNum(QModelIndex()));
     }
   }
